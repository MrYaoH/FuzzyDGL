...
from utils import *
from train import *
from Model.module_fuzzy import FuzzyDGL
...
if args.mode == 't':
    ...
    valid_train_flag = (ts_l <= val_time) * (none_mask_node_flag > 0)
...
valid_test_new_new_flag = valid_test_flag * is_new_node_edge
...
nn_test_rand_sampler = RandEdgeSampler((test_src_new_new_l, ), (test_dst_new_new_l, ))
...
model = FuzzyDGL(n_feat, e_feat, n_rules=N_RULES, agg=AGG,
...
train_val(train_val_data, model, args.mode, BATCH_SIZE, NUM_EPOCH, criterion, optimizer, early_stopper, ngh_finders, rand_samplers, logger)

# final testing
model.update_ngh_finder(full_ngh_finder)  # remember that testing phase should always use the full neighbor finder
test_acc, test_ap, test_f1, test_auc = eval_one_epoch('test for {} nodes'.format(args.mode), model, test_rand_sampler, test_src_l, test_dst_l, test_ts_l, test_label_l, test_e_idx_l)
...
if args.mode == 'i':
    test_new_new_acc, test_new_new_ap, test_new_new_f1, test_new_new_auc = eval_one_epoch('test for {} nodes'.format(args.mode), model, nn_test_rand_sampler, test_src_new_new_l, test_dst_new_new_l, test_ts_new_new_l, test_label_new_new_l, test_e_idx_new_new_l)
    logger.info('Test statistics: {} inductive nodes -- acc: {}, auc: {}, ap: {}'.format(args.mode, test_new_new_acc, test_new_new_auc,test_new_new_ap ))
    test_new_old_acc, test_new_old_ap, test_new_old_f1, test_new_old_auc = eval_one_epoch('test for {} nodes'.format(args.mode), model, test_rand_sampler, test_src_new_old_l, test_dst_new_old_l, test_ts_new_old_l, test_label_new_old_l, test_e_idx_new_old_l)
    logger.info('Test statistics: {} new-old nodes -- acc: {}, auc: {}, ap: {}'.format(args.mode, test_new_old_acc, test_new_old_auc, test_new_old_ap))

# save model
logger.info('Saving FuzzyDGL model ...')
torch.save(model.state_dict(), best_model_path)
logger.info('FuzzyDGL model saved')
