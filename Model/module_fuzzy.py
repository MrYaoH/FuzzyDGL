...
class TimeEncode(torch.nn.Module):
    def __init__(self, expand_dim, factor=5):
        ...
        self.phase = torch.nn.Parameter(torch.zeros(self.time_dim // 2).float())
    ...
    def forward(self, ts):
        ...
        inputs = torch.log(ts + 1)
        map_ts = torch.matmul(torch.reshape(inputs, [-1, 1]), torch.reshape(self.basis_freq, [1, -1]))
        map_ts += self.phase
...
class FuzzyDGL(torch.nn.Module):
    def __init__(self, n_feat, e_feat, n_rules, agg='tree',
        ...
        super(FuzzyDGL, self).__init__()
        self.logger = logging.getLogger(__name__)
        self.verbosity = verbosity
        self.logger.info('Fuzzy Attantion')
    ...
    def tmp_conv(self, src_idx_l, tgt_idx_l, cut_time_l, e_idx_l=None, test=False):
    ...
    def tmp_edge(self, src_idx_l, tgt_idx_l, cut_time_l, e_idx_l=None, test=False):
...
class RandomWalkAttention(nn.Module):
    ...
    '''
    def __init__(self, feat_dim, pos_dim, model_dim, out_dim, logger, walk_lens, n_rules, walk_pool='attn', mutual=False, n_head=8, dropout_p=0.1, walk_linear_out=False):
        '''
        ...
        # encode all types of features along each temporal walk across different fuzzy rules
        self.self_attention = TransformerEncoderLayer(d_model=self.feat_dim, nhead=2,
                                                      dim_feedforward=2 * self.feat_dim, dropout=self.dropout_p,
                                                      activation='relu')
        self.dropout = nn.Dropout(dropout_p)
        self.out_proj = nn.Sequential(nn.Linear(self.model_dim, self.out_dim), nn.ReLU(), self.dropout)

        self.Cons = torch.FloatTensor(size=(self.n_rules, self.feat_dim * walk_lens, self.feat_dim))
        self.Bias = torch.FloatTensor(size=(1, self.n_rules, self.feat_dim))
        self.Cs = torch.FloatTensor(size=(self.feat_dim * walk_lens, self.n_rules))
        ...
        self.logger.info('actual encoding dim: {} , attention dim: {}, attention heads: {}'.format(self.model_dim, self.attn_dim, self.n_head))
    ...
    def forward_one_node(self, hidden_embeddings, time_features, edge_features, position_features, masks=None):
        ...
        # setting mask
        masks = masks.reshape(batch * n_walk, -1).float()

        combined_features = self.aggregate(hidden_embeddings, time_features, edge_features, position_features)
        combined_features = combined_features.reshape(batch * n_walk, len_walk, -1)
        combined_features = torch.mul(combined_features, masks.unsqueeze(2))
        fuzzy_input = combined_features.reshape(batch * n_walk, -1)

        sim = torch.sum(-(fuzzy_input.unsqueeze(dim=-1) - self.Cs) ** 2 / (2 * self.Vs ** 2), dim=1)
        frs = F.softmax(sim, dim=-1)

        x_rep = fuzzy_input.unsqueeze(dim=1).expand([fuzzy_input.size(0), self.n_rules, fuzzy_input.size(1)])
        cons1 = torch.einsum('ijk,jkl->ijl', [x_rep, self.Cons])
        cons1 = cons1 + self.Bias
        path_out = torch.matmul(frs.unsqueeze(dim=1), cons1).squeeze(dim=1)

        # aggregation of path
        path_out_norm = F.normalize(path_out.reshape(batch, n_walk, -1), p=2, dim=-1)
        path_agg = self.self_attention(path_out_norm)
        node_emb = self.out_proj(path_agg).mean(dim=-2)
