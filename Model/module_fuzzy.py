...
class RandomWalkAttention(nn.Module):
    ...
    def __init__(self, feat_dim, pos_dim, model_dim, out_dim, logger, walk_lens, n_rules, walk_pool='attn', mutual=False, n_head=8, dropout_p=0.1, walk_linear_out=False):
        ...
        self.Cons = torch.FloatTensor(size=(self.n_rules, self.feat_dim * walk_lens, self.feat_dim))  # 这个应该是后续的线性投影部分
        self.Bias = torch.FloatTensor(size=(1, self.n_rules, self.feat_dim))
        self.Cs = torch.FloatTensor(size=(self.feat_dim * walk_lens, self.n_rules))  # 保存初始化聚类中心，也就是模糊隶属度函数的均值
    ...
    def forward_one_node(self, hidden_embeddings, time_features, edge_features, position_features, masks=None):
        ...
        # setting mask
        masks = masks.reshape(batch * n_walk, -1).float()  # [sum_walks, len_walks]
        ...
        fuzzy_input = combined_features.reshape(batch * n_walk, -1)  # 模糊输入的为一个路径的特征
        ...
        path_out = torch.matmul(frs.unsqueeze(dim=1), cons1).squeeze(dim=1)   # defuzzy  [sum_n_walk, dim]
